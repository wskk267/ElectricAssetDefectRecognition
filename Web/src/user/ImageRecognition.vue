    ...
    <div class="image-recognition">
                ...
                <el-card class="image-card">
                    <template #header>
                        <div class="card-header">
                            <span><el-icon><Picture /></el-icon> 图片识别</span>
                        </div>
                    </template>
                        ...
                        <div v-else class="image-wrapper">
                            ...
                            <div v-if="showBoundingBoxes && recognitionResults.length > 0 && !detailViewActive" class="bounding-boxes-overlay">
                                <div 
                                    ...
                                    :key="`bbox-${result.uniqueId || result.id}-${index}`"
                                    class="bounding-box"
                                    :style="getBboxStyle(result)"
                                    @click="showDetailedView(result)"
                                >
                                    <!-- 标签 -->
                                    <div class="box-label" 
                                        :class="{
                        ...
                        <div v-if="detailViewActive" class="detail-view-overlay" @click="closeDetailedView">
                            <div class="detail-view-container" @click.stop>
                                <div class="detail-view-header">
                                    <span>设备详情 #{{ selectedDetail.id }}</span>                    <el-button class="custom-btn return-btn" type="info" plain size="large" @click="closeDetailedView">
                        <el-icon class="btn-icon"><Back /></el-icon>返回
                    </el-button>
                                </div>
                                <div class="detail-view-content">
                                    <div class="detail-image-container">
                                        <div class="detail-image-wrapper">
                                            <img :src="detailImage" alt="裁剪视图" style="object-fit: contain;width: 100%; height: 100%;background-color:#1A1A2D;"/>
                                        </div>
                                    </div>
                                    <div class="detail-info">
                                        ...
                                        <p><strong>缺陷状态:</strong> 
                                            <el-tag :type="selectedDetail.defect_status === '缺陷' ? 'danger' : 'success'" size="small">
                                                {{ selectedDetail.defect_status }}
                                            </el-tag>
                                        </p>
                                        <p><strong>置信度:</strong> {{ (selectedDetail.confidence * 100).toFixed(1) }}%</p>
                                        <p><strong>中心位置:</strong> ({{ (selectedDetail.center.x * 100).toFixed(1) }}%, {{ (selectedDetail.center.y * 100).toFixed(1) }}%)</p>
                                        <p><strong>检测框尺寸:</strong> {{ (selectedDetail.width * 100).toFixed(1) }}% × {{ (selectedDetail.height * 100).toFixed(1) }}%</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div v-if="uploadedImage" class="image-controls">
                        <el-button 
                            ...
                            size="large" 
                            @click="resetImage">
                            <el-icon class="btn-icon"><RefreshRight /></el-icon>重新上传
                        </el-button>
                        <el-button 
                            class="custom-btn recognize-btn" 
                            type="success" 
                            :loading="recognizing" 
            ...
            <el-card class="results-card">
                ...
                <div class="results-list">
                    <div v-for="(result, index) in filteredResults" 
                         :key="`result-${result.uniqueId || result.id}-${index}`" 
                         class="result-item"
                         :class="{'result-selected': isResultSelected(result)}"
                         @click="showDetailedView(result)">
        ...
        <div class="right-panel">
            ...
            <el-card class="control-card">
                <template #header>
                    <div class="card-header">
                        <span><el-icon><Setting /></el-icon> 显示选项</span>
                    </div>
                </template>
                ...
                <div class="control-options">
                    ...
                    <div class="option-item">
                        ...
                        <el-slider
                            v-model="deviceConfidenceThreshold"
                            :min="0"
                            :max="100"
                            :step="1"
                            show-tooltip
                    ...
                    <div class="option-item">
                        ...
                        <el-slider
                            v-model="defectConfidenceThreshold"
                            :min="0"
                            :max="100"
                            :step="1"
                            show-tooltip
                            :format-tooltip="(val) => `${val}%`"
                            class="defect-threshold-slider"
                            style="margin: 10px 0;">
                        </el-slider>
                    </div>
                </div>
            ...
            <el-card class="control-card">
                <template #header>
                    <div class="card-header">
                        <span><el-icon><Menu /></el-icon> 设备类别筛选</span>
                    </div>
                </template>
...
.upload-area-wrapper {
    ...
    border-radius: 10px;
    border: 1px dashed rgba(0, 245, 255, 0.2); /* 减小边框 */
}
...
.image-card {
    ...
    height: 100%;
    min-height: 0; /* 确保可以收缩 */
    max-height: calc(100vh - 120px); /* 进一步限制高度 */
}
...
.image-container {
    ...
    flex: 1;
    min-height: 0; /* 允许收缩 */
    max-height: calc(100vh - 200px); /* 限制容器高度，防止超出页面 */
}
...
.preview-image {
    max-width: 100%;
    max-height: calc(100vh - 250px); /* 进一步限制高度 */
    width: auto;
    height: auto;
    object-fit: contain;
    display: block;
...
.results-card {
    flex: 1;
    min-height: 0; /* 允许结果卡片收缩 */
}

.results-list {
    height: 100%;
    overflow-y: auto;
    max-height: calc(100vh - 200px); /* 防止结果列表超出屏幕 */
...
.box-label {
    ...
    color: white;
    padding: 2px 5px;
    font-size: min(14px, 1.8vh);  /* 响应式字体大小 */
    white-space: nowrap;
    border-radius: 3px;
    pointer-events: none;
    max-width: 200px;
...
.label-inside {
    ...
    transform: none;
    border-radius: 4px;
    font-size: min(12px, 1.5vh); /* 内部标签稍小 */
}
...
.detail-info p:hover {
    background: rgba(0, 245, 255, 0.1);
    transform: translateX(3px); /* 减小悬停偏移 */
}
...
@media (max-width: 1200px) {
    ...
    .detail-image-container {
        flex: 1;
        min-height: 250px; /* 移动端保证足够的图片显示空间 */
        max-height: 40vh; /* 限制最大高度，留出空间给信息区域 */
    }
    
    .detail-image-wrapper {
        min-height: 200px; /* 移动端最小高度调整 */
    }
